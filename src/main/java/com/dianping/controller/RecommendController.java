...
public class RecommendController {
	...
	public String index(Model model ,@RequestParam(required=true,defaultValue="1")int pageNum,
			@RequestParam(required=true,defaultValue="10")int pageSize,@RequestParam(required=false)String category){
		PageHelper.startPage(pageNum, pageSize);
		List<RankShopInfo> findRecommend = dianpingDao.findRecommend(category);
